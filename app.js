...
async function fetchAllAssignments() {
  const { data, error } = await supabase.from("shelf_assignments").select("*");
...
async function toggleAssignment(code, shelf) {
  ...
  try {
    if (isUnassigning) {
      const { error } = await supabase.from("shelf_assignments").delete().eq("product_code", code);
      if (error) throw error;
    } else {
      const { error } = await supabase.from("shelf_assignments").upsert(
        {
          product_code: code,
          shelf,
          updated_by: state.me || "익명",
